...
export class SmartPoller {
   ...
   * 检查是否应该退出轮询
   */
  private shouldExitPolling(pollingStatus: PollingStatus): {
    shouldExit: boolean;
    reason: string;
  } {
  ...
  async poll<T>(
    ...
    while (true) {
      ...
      try {
        ...
        // 详细日志
        logger.info(
          `轮询 ${this.pollCount}/${this.options.maxPollCount}: status=${status.status}(${this.getStatusName(status.status)}), failCode=${status.failCode || "none"}, items=${status.itemCount}, elapsed=${elapsedTime}s, finish_time=${status.finishTime || 0}, stable=${this.stableItemCountRounds}/${this.options.stableRounds}`,
        );

        // 如果有结果生成，记录详细信息
        if (status.itemCount > 0) {
          logger.info(
            `检测到${this.options.type === "image" ? "图片" : "视频"}生成: 数量=${status.itemCount}, 状态=${this.getStatusName(status.status)}`,
          );
        }
        ...
        if (this.pollCount % 30 === 0) {
          logger.info(
            `${this.options.type === "image" ? "图像" : "视频"}生成进度: 第 ${this.pollCount} 次轮询，状态: ${this.getStatusName(status.status)}，已等待 ${elapsedTime} 秒...`,
          );
        }

        // 计算下次轮询间隔
        const nextInterval = this.getSmartInterval(
          status.status,
          status.itemCount,
        );
        if (nextInterval > 0) {
        ...
        if (isRetryableError && this.pollCount < this.options.maxPollCount) {
          logger.warn(
            `轮询过程中发生网络错误 (${error.code || error.message})，等待后继续轮询...`,
          );
