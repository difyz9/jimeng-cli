...
export class JimengErrorHandler {
  ...
  static handleApiResponse(
    ...
    logger.error(
      `${context}失败: ret=${ret}, errmsg=${errmsg}${historyId ? `, historyId=${historyId}` : ""}`,
    );
    ...
    switch (ret) {
      case "1015":
        throw new APIException(
          ...
          `[登录失效]: ${errmsg}。请重新获取refresh_token并更新配置`,
        );
      ...
      case "34010105":
        throw new APIException(
          ...
          `[登录验证失败]: ${errmsg} (错误码: ${ret})`,
        );

      default:
        throw new APIException(
          EX.API_REQUEST_FAILED,
          `[${operation}失败]: ${errmsg} (错误码: ${ret})`,
        );
    }
  }
  ...
  static handleNetworkError(
    ...
    logger.error(
      `${context}网络错误 (尝试 ${retryCount + 1}/${maxRetries + 1}): ${error.message}`,
    );

    if (error.code === "ECONNABORTED") {
      throw new APIException(
        EX.API_REQUEST_FAILED,
        `[请求超时]: ${context}超时，请稍后重试`,
      );
    }
    ...
    if (error.response?.status >= 500) {
      throw new APIException(
        EX.API_REQUEST_FAILED,
        `[服务器错误]: 即梦服务器暂时不可用 (${error.response.status})`,
      );
    }
  ...
  static handleGenerationFailure(
    ...
    if (itemCount > 0) {
      logger.warn(message);
      logger.info(
        `${typeText}生成部分失败，但已获得 ${itemCount} 个结果，将返回现有结果`,
      );
      return false; // 不抛出异常
    }
    ...
    throw new APIException(
      exception,
      `${typeText}生成失败，状态码: ${status}${failCode ? `，错误码: ${failCode}` : ""}`,
    );
  }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          logger.warn(
            `${context}失败 (尝试 ${attempt + 1}/${maxRetries + 1}): ${error.message}`,
          );
...
export const handleGenerationFailure =
  JimengErrorHandler.handleGenerationFailure;
