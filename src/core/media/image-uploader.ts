...
export async function uploadImageBuffer(
  ...
  try {
    logger.info(
      `开始上传图片Buffer... (isInternational: ${regionInfo.isInternational})`,
    );
    ...
    const service_id = regionInfo.isInternational
      ? tokenResult.space_name
    ...
    try {
      ...
      throw new Error(
        `网络请求失败 (${applyUrlHost}): ${fetchError.message}. 请检查: 1) 网络连接是否正常 2) 是否需要配置代理 3) DNS是否能解析该域名`,
      );
    }
    ...
    try {
      ...
      logger.error(`错误详情: ${fetchError.message}`);
      throw new Error(
        `图片上传网络请求失败 (${uploadHost}): ${fetchError.message}. 请检查网络连接`,
      );
    }
    ...
    try {
      ...
      logger.error(`错误详情: ${fetchError.message}`);
      throw new Error(
        `提交上传网络请求失败 (${applyUrlHost}): ${fetchError.message}. 请检查网络连接`,
      );
    }
    ...
    logger.info(
      `图片上传完成: ${fullImageUri} (${width}x${height}, ${format})`,
    );
