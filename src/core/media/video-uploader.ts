...
export async function uploadVideoBuffer(
  ...
  try {
    ...
    logger.info(
      `开始上传视频Buffer... (size=${fileSize}, isInternational=${regionInfo.isInternational})`,
    );
    ...
    try {
      ...
      throw new Error(
        `视频上传申请网络请求失败 (${vodHost}): ${fetchError.message}`,
      );
    }

    if (applyResponse.status < 200 || applyResponse.status >= 300) {
      const errorText =
        typeof applyResponse.data === "string"
          ? applyResponse.data
          : JSON.stringify(applyResponse.data);
      throw new Error(
        `申请视频上传权限失败: ${applyResponse.status} - ${errorText}`,
      );
    }
    ...
    if (applyResult?.ResponseMetadata?.Error) {
      throw new Error(
        `申请视频上传权限失败: ${JSON.stringify(applyResult.ResponseMetadata.Error)}`,
      );
    }
    ...
    if (!storeInfo) {
      throw new Error(
        `获取视频上传存储信息失败: ${JSON.stringify(uploadNode)}`,
      );
    }
    ...
    logger.info(
      `获取视频上传节点成功: host=${uploadHost}, vid=${vid}, type=${uploadNode.Type}`,
    );
    ...
    try {
      ...
      throw new Error(
        `视频文件上传网络请求失败 (${uploadHost}): ${fetchError.message}`,
      );
    }
    ...
    if (uploadResponse.status < 200 || uploadResponse.status >= 300) {
      ...
      throw new Error(
        `视频文件上传失败: ${uploadResponse.status} - ${errorText}`,
      );
    }
    ...
    if (uploadData?.code !== 2000) {
      throw new Error(
        `视频文件上传失败: code=${uploadData?.code}, message=${uploadData?.message}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `提交视频上传网络请求失败 (${vodHost}): ${fetchError.message}`,
      );
    }

    if (commitResponse.status < 200 || commitResponse.status >= 300) {
      const errorText =
        typeof commitResponse.data === "string"
          ? commitResponse.data
          : JSON.stringify(commitResponse.data);
      throw new Error(
        `提交视频上传失败: ${commitResponse.status} - ${errorText}`,
      );
    }
    ...
    if (commitResult?.ResponseMetadata?.Error) {
      throw new Error(
        `提交视频上传失败: ${JSON.stringify(commitResult.ResponseMetadata.Error)}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `提交视频上传响应缺少结果: ${JSON.stringify(commitResult)}`,
      );
    }
    ...
    if (videoMeta?.Duration && videoMeta.Duration > MAX_VIDEO_DURATION) {
      throw new Error(
        `视频时长 ${videoMeta.Duration.toFixed(2)}s 超过限制 (最大 ${MAX_VIDEO_DURATION}s)`,
      );
    }

    logger.info(
      `视频上传完成: vid=${result.Vid}, ${videoMeta?.Width}x${videoMeta?.Height}, ${videoMeta?.Duration}s, ${videoMeta?.Format}/${videoMeta?.Codec}`,
    );
